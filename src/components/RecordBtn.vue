...
<template>
  ...
  <button :class="`btn-icon is-${color}`" :disabled="recording" @click="startDictation">
    <span v-if="!retry && !recording" class="svg-icon" title="Record">
    ...
    <span v-else-if="recording" class="svg-icon" title="Recording...">
    ...
    <span v-else-if="retry || error" class="svg-icon" title="Try again">
    ...
    <div v-if="recording && showPulse" class="pulse" />
  </button>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.recognition.onresult = function (e) {
      ...
      const altArray = [];
      for (let i = 0; i < e.results[0].length; i + 1) {
